      ...
      <div className="max-w-7xl mx-auto mt-8">
        <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-6 border-t-4 border-emerald-800">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-emerald-800">Potongan Tetap Ustadz</h1>
              <p className="text-emerald-700 mt-1 text-sm">Kelola potongan tetap per ustadz</p>
            </div>
            <div className="flex gap-2">
              ...
              <Link href="/hrd/fixedCuts" className="px-4 py-2 rounded-xl bg-emerald-800 text-white">Tambah Data</Link>
          ...
          <div className="overflow-x-auto rounded-xl border border-emerald-100 mt-6">
            <table className="w-full text-sm">
              ...
              <tbody>
                {items.map((it) => (
                  <tr key={it._id || it.nip} className="odd:bg-white even:bg-emerald-50/50">
                ...
                {items.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-4 py-6 text-center text-emerald-800 bg-emerald-50/70 italic">Tidak ada data</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          <div className="flex items-center justify-between mt-4">
            ...
            <button onClick={exportToExcel} className="px-4 py-2 rounded-xl bg-emerald-800 text-white flex items-center gap-2"><FileSpreadsheet className="w-4 h-4" />Export to Excel</button>
          </div>

          {importOpen && (
            <div className="mt-6 p-4 border rounded-xl bg-emerald-50/70">
              <div className="flex items-center gap-2">
                <input type="file" accept=".xlsx,.csv" onChange={(e) => setImportFile(e.target.files?.[0] || null)} />
                <button onClick={handleImport} className="px-4 py-2 rounded-xl bg-emerald-800 text-white">Proses Import</button>
                <button onClick={() => { setImportOpen(false); setImportFile(null); }} className="px-4 py-2 rounded-xl bg-gray-200 text-emerald-800">Tutup</button>
              </div>
              <p className="text-xs text-emerald-700 mt-2">Format kolom: NIP, Potongan</p>
            </div>
          )}
