...
import { getSettings, upsertSetting } from "./action";
...
export default function SettingsIndexPage() {
  ...
  const add = async () => {
    ...
    setLoading(true);
    const res = await upsertSetting(keyName.trim(), value);
    setLoading(false);
    if (res.success) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-teal-50 p-6">
      <div className="max-w-4xl mx-auto mt-10 bg-white rounded-2xl shadow-xl p-6 border-t-4 border-blue-800">
        <h1 className="text-2xl font-bold text-blue-800">Pengaturan</h1>
        <p className="text-blue-700 mt-1 text-sm">Kelola pengaturan aplikasi</p>
        ...
        <div className="mt-6 rounded-xl border border-blue-100 overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.map((it) => (
                <tr key={it.id || it.key} className="odd:bg-white even:bg-blue-50/50">
