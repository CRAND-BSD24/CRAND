      ...
      <div className="max-w-7xl mx-auto mt-8">
        <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-6 border-t-4 border-blue-800">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-blue-800">Potongan Tetap</h1>
              <p className="text-blue-700 mt-1 text-sm">Kelola potongan tetap</p>
            </div>
            <div className="flex gap-2">
              <Link href="/hrd/fixedCuts/create" className="px-4 py-2 rounded-xl bg-blue-800 text-white">Tambah Baru</Link>
            </div>
          </div>
          ...
          <div className="overflow-x-auto rounded-xl border border-blue-100 mt-6">
                ...
                {items.map((it, idx) => (
                  <tr key={it.id || `${it.name}-${idx}`} className="odd:bg-white even:bg-blue-50/50">
                    ...
                    <td className="px-3 py-2">
                      <Link href={`/hrd/fixedCuts/create?edit=${encodeURIComponent(it.id || "")}`} className="text-blue-800">Edit</Link>
                    </td>
                  </tr>
                ))}
                {items.length === 0 && (
                  <tr>
                    <td colSpan={4} className="px-4 py-6 text-center text-blue-800 bg-blue-50/70 italic">Tidak ada data</td>
                  </tr>
                )}
              </tbody>
            </table>
