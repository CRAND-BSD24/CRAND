        ...
        <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-8 border-t-4 border-emerald-800">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 sm:mb-8 gap-4">
            <div>
              ...
              <p className="text-emerald-700 mt-1 pl-8 sm:pl-11 text-sm sm:text-base">Kelola gaji pokok per jabatan</p>
            </div>
            <button onClick={() => setImportOpen(true)} className="bg-emerald-800 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-xl hover:bg-emerald-700 transition-all duration-300 flex items-center gap-2 shadow-md">
          ...
          <div className="overflow-x-auto rounded-xl border border-emerald-100">
            <table className="w-full text-sm">
              ...
              <tbody>
                {current.map((job) => (
                  <tr key={job} className="odd:bg-white even:bg-emerald-50/50">
                    <td className="px-3 py-2">{job}</td>
                    <td className="px-3 py-2">
                      <div className="flex justify-end">
                        <Link href={`/hrd/salaries/create?job=${encodeURIComponent(job)}`} className="px-3 py-1.5 bg-emerald-800 text-white rounded-lg hover:bg-emerald-700 transition-all text-sm">
                ...
                {current.length === 0 && (
                  <tr>
                    <td colSpan={2} className="px-4 py-6 text-center text-emerald-800 bg-emerald-50/70 italic">Tidak ada data jabatan</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between mt-6">
              <div className="text-xs sm:text-sm bg-emerald-50 text-emerald-800 px-3 py-2 rounded-lg border border-emerald-200">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1} className={`p-2 rounded-lg ${currentPage === 1 ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-emerald-800 text-white'}`}>
                  <ChevronLeft className="w-4 h-4" />
                </button>
                <div className="flex gap-1">
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                    <button key={page} onClick={() => setCurrentPage(page)} className={`min-w-[32px] h-8 rounded-lg text-sm ${currentPage === page ? 'bg-emerald-800 text-white font-bold' : 'bg-gray-100 hover:bg-emerald-100 text-gray-700 hover:text-emerald-800'}`}>{page}</button>
                  ))}
                </div>
                <button onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className={`p-2 rounded-lg ${currentPage === totalPages ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-emerald-800 text-white'}`}>
          ...
          <div className="mt-6">
            <button onClick={exportToCSV} className="px-4 py-2 rounded-lg bg-emerald-800 text-white flex items-center gap-2">
      ...
      {importOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl p-6 border-t-4 border-emerald-800">
            <h2 className="text-xl sm:text-2xl font-bold text-emerald-800">Impor Data Gaji Pokok</h2>
            ...
            <p className="text-emerald-700 mt-3">Pastikan anda telah membuat file impor dengan format .xls atau .xlsx sesuai dengan template yang disediakan. Untuk mendapatkan template, silakan klik tombol download format import dibawah.</p>
            <div className="mt-5 flex flex-col sm:flex-row gap-2">
              <button onClick={downloadTemplate} className="px-4 py-2 rounded-xl bg-emerald-800 text-white">Download Format Import</button>
