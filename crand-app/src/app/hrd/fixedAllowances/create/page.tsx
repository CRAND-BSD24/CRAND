...
import { createFixedAllowance, getFixedAllowanceById } from "../action";
import { toast } from "sonner";

export default function CreateFixedAllowancePage() {
  ...
  const [applyAt, setApplyAt] = useState<"Check In" | "Check Out">("Check In");
  ...
  useEffect(() => {
    (async () => {
      if (editId) {
        const row = await getFixedAllowanceById(editId);
        if (row) {
          setName(row.name || "");
          setPriority(String(row.priority ?? ""));
          setType((row.type as any) === "Bulanan" ? "Bulanan" : "Harian");
          setApplyAt((row.apply_at as any) === "Check Out" ? "Check Out" : "Check In");
        }
      }
    })();
  }, [editId]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await createFixedAllowance({ name: nm, priority: pr, type, apply_at: applyAt });
      if (res.success) {
        toast.success("Data tersimpan");
        router.push("/hrd/fixedAllowances");
      } else {
