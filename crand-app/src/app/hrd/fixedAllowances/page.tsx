      ...
      <div className="max-w-7xl mx-auto mt-8">
        <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-6 border-t-4 border-emerald-800">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-emerald-800">Tunjangan Tetap</h1>
              <p className="text-emerald-700 mt-1 text-sm">Kelola tunjangan tetap</p>
            </div>
            <div className="flex gap-2">
              <Link href="/hrd/fixedAllowances/create" className="px-4 py-2 rounded-xl bg-emerald-800 text-white">Tambah Baru</Link>
            </div>
          </div>
                ...
                {items.map((it, idx) => (
                  <tr key={it.id || `${it.name}-${idx}`} className="odd:bg-white even:bg-emerald-50/50">
                    ...
                    <td className="px-3 py-2">
                      <Link href={`/hrd/fixedAllowances/create?edit=${encodeURIComponent(it.id || "")}`} className="text-emerald-800">Edit</Link>
                    </td>
                  </tr>
                ))}
                {items.length === 0 && (
