...
Selalu jawab dalam Bahasa Indonesia yang formal dan sopan.`;
    ...
    if (error instanceof Error) {
      ...
      if (errorMessage.includes("404 Not Found") || errorMessage.includes("not found for API version")) {
        return "❌ Maaf, layanan AI sementara tidak tersedia atau model tidak ditemukan. Silakan coba lagi beberapa saat lagi.";
      }
      if (errorMessage.includes("API key not valid") || errorMessage.includes("PERMISSION_DENIED")) {
        return "❌ Maaf, konfigurasi kunci API AI tidak valid. Silakan hubungi admin untuk memperbarui pengaturan.";
      }
    }
