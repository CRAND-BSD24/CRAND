...
import { getMyBisyaroh, BisyarohSlip } from "./action";
...
export default function BisyarohPage() {
  ...
  if (loading) {
    return <div className="p-8 text-center text-blue-800">Memuat data bisyaroh...</div>;
  }
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {filteredSlips.length === 0 ? (
          ...
          filteredSlips.map((slip) => (
            <Card key={slip._id} className="border-blue-100 hover:shadow-md transition-shadow">
              <CardHeader className="bg-blue-50/50 pb-4">
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-2 text-blue-700">
                    <Calendar className="h-4 w-4" />
                    <span className="font-medium text-sm">{slip.month_name} {slip.year}</span>
                  </div>
                ...
                <CardDescription className="text-xs">
                  Diterbitkan: {new Date(slip.date).toLocaleDateString('id-ID')}
                </CardDescription>
              </CardHeader>
