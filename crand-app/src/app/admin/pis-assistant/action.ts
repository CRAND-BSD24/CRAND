...
async function getDatabaseContextForAdmin(
  db: any,
  ...
  if (
    normalizedQuestion.includes("santri bernama") ||
    normalizedQuestion.includes("info santri")
  ) {
    let studentName;
    ...
    if (studentName) {
      ...
      const student = await db
        .collection("students")
        .findOne({ name: { $regex: `.*${studentName}.*`, $options: "i" } });
      if (!student)
        return `Tidak ada data santri dengan nama "${studentName}".`;

      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const achievements: any[] = await db
        .collection("achievements")
        .find({ studentId: new ObjectId(student._id) })
        .sort({ date: -1 })
        .limit(3)
        .toArray();
      const academicRecords: AcademicRecord[] = await db
      ...
      context += `Kelas: ${student.class}, Status: ${
        student.status
      }, Jenis Kelamin: ${getGenderDisplay(student.gender)}\n`;
      if (student.enrollmentDate)
        context += `Tanggal Masuk: ${new Date(
      ...
      if (achievements.length > 0) {
        ...
        achievements.forEach(
          (ach: any, i: number) =>
            (context += `${i + 1}. ${ach.title} (${ach.category}, ${
              ach.level
            }, ${
              ach.date ? new Date(ach.date).toLocaleDateString("id-ID") : "N/A"
            })\n`)
        );
      }
      if (academicRecords.length > 0) {
        const rec: AcademicRecord = academicRecords[0];
        context += `Akademik (${rec.semester} ${rec.year}): Rata-rata ${
          rec.average?.toFixed(2) || "N/A"
        }, Peringkat ${rec.rank || "N/A"}\n`;
      }
  ...
  if (
    normalizedQuestion.includes("ustadz bernama") ||
    normalizedQuestion.includes("info ustadz")
  ) {
    let teacherName;
    ...
    if (teacherName) {
      ...
      if (!teacher)
        return `Tidak ada data ustadz dengan nama "${teacherName}".`;
  ...
  if (
    normalizedQuestion.includes("jumlah santri") ||
    normalizedQuestion.includes("total santri")
  ) {
    console.log("[Admin Context] Fetching context for total students");
    const totalStudents = await db.collection("students").countDocuments();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const statusCounts: any[] = await db
      .collection("students")
  ...
  if (
    normalizedQuestion.includes("jumlah ustadz") ||
    normalizedQuestion.includes("total ustadz")
  ) {
    ...
    statusCounts.forEach(
      (s: any) => (context += `${s._id || "Aktif"}: ${s.count}\n`)
    );
    return context;
  }
  ...
  if (
    normalizedQuestion.includes("kelas") ||
    normalizedQuestion.includes("tingkat")
  ) {
    console.log("[Admin Context] Fetching context for class distribution");
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const classDistribution: any[] = await db
      .collection("students")
      .aggregate([
    ...
    if (classDistribution.length === 0)
      return "Tidak ada data distribusi kelas.";
    let context = "Konteks Distribusi Kelas:\n";
    let totalSantri = 0;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    classDistribution.forEach((cls: any) => {
      context += `Kelas ${cls._id}: ${cls.count}\n`;
...
export async function processQuestion(question: string): Promise<string> {
  ...
  try {
    ...
    const dbContext = await getDatabaseContextForAdmin(
      db,
      normalizedQuestion,
      question
    );
...
Selalu jawab dalam Bahasa Indonesia yang baik dan sopan.`;
    ...
    if (dbContext) {
      ...
      console.log(
        "[Admin Action] No specific database context found for Gemini."
      );
    }
    ...
    const chat = model.startChat({
      ...
      history: [
        { role: "user", parts: [{ text: systemPrompt }] },
        {
          role: "model",
          parts: [
            { text: "Baik, saya PIS Assistant (Admin). Siap membantu Anda." },
          ],
        },
      ],
    });
    ...
    console.log(
      "[Admin Action] Gemini Raw Response:",
    ...
    console.error(
      "[Admin Action] Error processing question with Gemini:",
      error
    );
